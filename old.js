class DataVisualization {
  ...
  static svg  = d3.select("svg");
  static width  = parseInt(DataVisualization.svg.attr("width"));
  static height = parseInt(DataVisualization.svg.attr("height"));
  static hypotenuse = Math.sqrt(DataVisualization.width * DataVisualization.width + DataVisualization.height * DataVisualization.height);
  ...
  static scales = {
    ...
    segments: d3.scaleLinear().domain([0, DataVisualization.hypotenuse]).range([1, 10]) // used to scale number of segments per line
  };
  static g = { // have these already created for easier drawing
    basemap:  DataVisualization.svg.select("g#basemap"),
    flights:  DataVisualization.svg.select("g#flights"),
    airports: DataVisualization.svg.select("g#airports"),
    voronoi:  DataVisualization.svg.select("g#voronoi")
  };
  static tooltip = d3.select("text#tooltip");

  constructor(start_dataset) {
    this.map = d3.json(DataVisualization.urls.map).then(this.drawMap); // load and draw base map
    this.promises = [ // load the airport and flight data together
        d3.csv(DataVisualization.urls.airports, this.typeAirport), 
        d3.csv(start_dataset,  this.typeFlight)
    ];
    Promise.all(this.promises).then(this.initSetup);
  } 
  ...
  initSetup(values) { // process airport and flight data
    ...
    this.drawFlights(airports, flights);
    this.drawPolygons(airports);
  }
  ...
  drawMap(map) { // DRAW UNDERLYING MAP
    ...
    DataVisualization.g.basemap.append("path") // draw base map
      .datum(land)
      .attr("class", "land")
      .attr("d", path);
  
    DataVisualization.g.basemap.append("path") // draw interior borders
      .datum(topojson.mesh(map, map.objects.states, (a, b) => a !== b))
      .attr("class", "border interior")
      .attr("d", path);
  
    DataVisualization.g.basemap.append("path") // draw exterior borders
      .datum(topojson.mesh(map, map.objects.states, (a, b) => a === b))
  ...
  drawAirports(airports) {
    console.log("airports: " + airports.length);
    DataVisualization.g.airports.selectAll("path.flight").remove(); 
    DataVisualization.g.airports.selectAll("circle.airport") // draw airport bubbles
  ...
  typeFlight(flight) { // see flights.csv, convert count to number
    flight.count = parseInt(flight.FLIGHTCOUNT);
    return flight;
  }

  typeAirport(airport) { //see airports.csv, convert gps coordinates to number and init degree
    airport.longitude_deg = parseFloat(airport.longitude_deg);
    airport.latitude_deg  = parseFloat(airport.latitude_deg);
  
    const coords = DataVisualization.projection([airport.longitude_deg, airport.latitude_deg]); // use projection hard-coded to match topojson data
    airport.x = coords[0];
    airport.y = coords[1];
  ...
  drawFlights(airports, flights) {
    DataVisualization.g.flights.selectAll("path.flight").remove(); 
     ...
     let layout = d3.forceSimulation() // https://github.com/d3/d3-force
       ...
       .force(3, d3.forceManyBody() // nearby nodes attract each other
         .strength(charge) 
         .distanceMax(scales.airports.range()[1] * 3)
       )
     
       .force(0.3, d3.forceLink() // edges want to be as short as possible, prevents too much stretching
         .strength(link)
         .distance(0)
       )
       .on("tick", function(d) {
